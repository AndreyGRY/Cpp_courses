...
void motivation() {
	...
	UNUSED(citer); UNUSED(citer2); UNUSED(citer3);
}
...
void example_2() {
	...
	auto s2 = (double)sum(1.0, 2.0); // s2 type ?

	UNUSED(s); UNUSED(f); UNUSED(af); UNUSED(str); UNUSED(astr); UNUSED(s2);
}
...
void example_3() {
	...
	UNUSED(aci); UNUSED(acrefi); UNUSED(avf); UNUSED(aptr); UNUSED(acptr); UNUSED(acptrc);
}
